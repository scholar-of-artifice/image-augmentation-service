...
def test_write_numpy_array_to_image_file(tmp_path, monkeypatch):
    ...
    calculated_file_path = write_numpy_array_to_image_file(
        data=input_numpy_array, file_name=file_name, destination_volume=destination
    )
    #
...
def test_write_numpy_array_to_image_file_to_invalid_file_path_raises_ValueError(
