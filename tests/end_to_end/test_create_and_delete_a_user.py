import uuid

import pytest
from fastapi import status

pytestmark = pytest.mark.asyncio

async def test_create_and_delete_a_user(http_client):
    """
    1. A user is created successfully.
    2. The same user is deleted successfully.
    """
    external_id = str(uuid.uuid4())
    headers = {"X-External-User-ID": external_id}
    # --- CREATE A USER ---
    create_user_response = await http_client.post(url="/users-api/users", headers=headers)
    assert create_user_response.status_code == status.HTTP_201_CREATED
    create_user_response_json = create_user_response.json()
    # --- DELETE A USER ---
    user_id = create_user_response_json["id"]
    delete_response = await http_client.delete(url=f"/users-api/users/{user_id}", headers=headers)
    assert delete_response.status_code == status.HTTP_204_NO_CONTENT
    # --- CHECK THE USER IS FULLY GONE ---
    verify_response = await http_client.post(url="/users-api/sign-in", headers=headers)
    assert verify_response.status_code == status.HTTP_404_NOT_FOUND

