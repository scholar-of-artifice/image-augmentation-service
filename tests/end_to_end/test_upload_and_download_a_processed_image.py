...
async def test_upload_and_download_a_processed_image(http_client):
    ...
    processed_image_id = upload_response.json()["processed_image_id"]
    download_response = await http_client.get(
        headers=headers,
        url=f"/image-api/processed-image/{processed_image_id}/"
    )
    assert download_response.status_code == status.HTTP_200_OK
    assert isinstance(download_response.content, bytes)
    # TODO: check image contents? idk maybe